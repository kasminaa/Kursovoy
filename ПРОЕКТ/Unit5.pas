...
begin
MPlayer.Close; //уничтожаем старый поток
if not FileExists(ExtractFilePath(Application.ExeName) + 'hip-hop_wmv.wmv') then
begin
ShowMessage('Файла 1.avi в папке с программой нет');
exit;
end;
MPlayer.FileName:=ExtractFilePath(Application.ExeName) + 'hip-hop_wmv.wmv'; //присваеваем значение имени файла(имя файла тут - это его полный путь).
MPlayer.Open; //создаем из файла поток
MPlayer.Play;
end;
